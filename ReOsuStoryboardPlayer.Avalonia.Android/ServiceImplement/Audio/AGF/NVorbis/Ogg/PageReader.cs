...
namespace NVorbis.Ogg
{
    class PageReader : PageReaderBase, IPageData
    {
        ...
        private static byte[][] ReadPackets(int packetCount, byte[] segments, byte[] dataBuffer)
        {
            var list = new byte[packetCount][];
            var listIdx = 0;
            var dataIdx = 0;
            var size = 0;
            ...
            for (var i = 0; i < segments.Length; i++)
            {
                ...
                if (seg < 255)
                {
                    if (size > 0)
                    {
                        list[listIdx++] = ArrayEx.CopyPart(dataBuffer, dataIdx, size);
                        dataIdx += size;
                    }
                    size = 0;
                }
        ...
        {
            return System.Threading.Monitor.IsEntered(_readLock);
        }

        public override bool Release()
        {
            if (System.Threading.Monitor.IsEntered(_readLock))
            {
        ...
        protected override bool AddPage(int streamSerial, byte[] pageBuf, bool isResync)
        {
            ...
            _packets = ReadPackets(PacketCount, ArrayEx.CopyPart(pageBuf, 27, pageBuf[26]), ArrayEx.CopyPart(pageBuf, 27 + pageBuf[26], pageBuf.Length - 27 - pageBuf[26]));
