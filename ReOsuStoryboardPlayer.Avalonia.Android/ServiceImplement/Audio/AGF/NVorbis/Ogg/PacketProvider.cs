...
namespace NVorbis.Ogg
{
    class PacketProvider : Contracts.IPacketProvider, IPacketReader
    {
        ...
        public struct PreviousPageInfo
        {
            public long lastPageGranulePos;
            public int lastPagePacketLength;
            public int firstRealPacket;

            public PreviousPageInfo(long lastPageGranulePos, int lastPagePacketLength, int firstRealPacket)
            {
                this.lastPageGranulePos = lastPageGranulePos;
                this.lastPagePacketLength = lastPagePacketLength;
                this.firstRealPacket = firstRealPacket;
            }
        }
        ...
        private PreviousPageInfo GetPreviousPageInfo(int pageIndex, GetPacketGranuleCount getPacketGranuleCount)
        {
            if (pageIndex > 0)
            {
                ...
                if (_reader.GetPage(pageIndex - 1, out var lastPageGranulePos, out _, out _, out var isContinued, out var lastPacketCount, out _))
                {
                    ...
                    return new PreviousPageInfo(lastPageGranulePos, lastPagePacketLength, isContinued ? 1 : 0);
                }
                throw new System.IO.InvalidDataException("Could not get preceding page?!");
            }
            else
            {
                return new PreviousPageInfo(0, 0, 0);
            }
        }

        public struct TargetPageInfo
        {
            public long[] gps;
            public long endGP;

            public TargetPageInfo(long[] gps, long endGP)
            {
                this.gps = gps;
                this.endGP = endGP;
            }
        ...
        private TargetPageInfo GetTargetPageInfo(int pageIndex, int firstRealPacket, int lastPagePacketLength, GetPacketGranuleCount getPacketGranuleCount)
        {
            ...
            return new TargetPageInfo(gps, endGP);
        }
        ...
        private int FindPacket(int pageIndex, int preRoll, ref long granulePos, GetPacketGranuleCount getPacketGranuleCount)
        {
            ...
            var r = GetPreviousPageInfo(pageIndex, getPacketGranuleCount);
            var lastPageGranulePos = r.lastPageGranulePos;
            var lastPagePacketLength = r.lastPagePacketLength;
            var firstRealPacket = r.firstRealPacket;

            // now get the info on the target page
            var r2 = GetTargetPageInfo(pageIndex, firstRealPacket, lastPagePacketLength, getPacketGranuleCount);
            var gps = r2.gps;
            var endGP = r2.endGP;
        ...
        byte[] IPacketReader.GetPacketData(int pagePacketIndex)
        {
