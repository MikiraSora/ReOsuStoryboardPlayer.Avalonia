...
using System.Linq;
using ReOsuStoryboardPlayer.Avalonia.Browser.ServiceImplement.Parameters;
...
internal static class CommandParser
{
    ...
    public static BrowserParameters Parse(string args, out string cmdName)
    {
        var array = args.Split(' ');
        cmdName = array.First();
        var arg = string.Join(" ", array.Skip(1));
        if (_parser.TryDivide(arg, out var p))
            return new BrowserParameters(p);
    ...
    public class ParamParserV2
    {
        ...
        public bool TryDivide(string args, out BrowserParameters p)
        {
            p = new BrowserParameters();
